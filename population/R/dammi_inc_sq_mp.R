datt <- rio::import("C:/Users/Yana/Downloads/f_pls_pls 3.xlsx")
save(datt, file = "rawdata6.RData")
load("rawdata6.RData")
library(dplyr)
library(data.table)
library(ggplot2)
...
data4lm <-datt %>% select(-c(1,3:5,9:14))%>%
...
sov_model_11 <- lm(formula = pop_log-pop_log_lag~ 0+log_mp+smp+oblast_center+CityStatus+RaionCenter+year+soviet_true+URAL+YUZHNY+DALNEV+PRIVOL+SEVER_ZAPAD+SIBIRSKII+TSENTRALNY+HBR+LBR,#+smp+spop,
                  data = data4lm)
summary(sov_model_11)

sov_model_22 <- lm(formula = pop_log-pop_log_lag~ 0+log_mp+smp+oblast_center+CityStatus+RaionCenter+year+soviet_true+URAL+YUZHNY+DALNEV+PRIVOL+SEVER_ZAPAD+SIBIRSKII+HBR+LBR,#+smp+spop,
                  data = data4lm)
summary(sov_model_22)
